...
export default function MusicPicker({ occasion, value, onChange }) {
  ...
  return (
    <section className="cl-music-picker">
      ...
      {mode === 'library' ? <>
        <div className="cl-music-categories"><button type="button" className={category === 'recommended' ? 'is-selected' : ''} onClick={() => setCategory('recommended')}>✨ Pour cette occasion</button>{MUSIC_CATEGORIES.map((item) => <button key={item.id} type="button" className={category === item.id ? 'is-selected' : ''} onClick={() => setCategory(item.id)}>{item.emoji} {item.label}</button>)}</div>
