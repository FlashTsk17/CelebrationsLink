    ...
    try {
      if (mode === 'signup') {
        ...
        if (!data.session) {
          ...
          navigate('/membre/espace', { replace: true })
        }
      } else {
      ...
      setError(submissionError?.message || 'Impossible de continuer.')
    } finally {
      setLoading(false)
    }
  }
    ...
    <main className="cl-shell">
      <div className="cl-container">
        <section className="cl-panel">
          ...
          <p>{mode === 'signup' ? 'Garde tes créations, retrouve-les plus tard et utilise CélébrationsLink sur plusieurs appareils.' : 'Retrouve ton espace personnel et tes créations.'}</p>
          ...
          <button className="cl-secondary-button" type="button" onClick={() => { setError(''); setMessage(''); setMode(mode === 'signup' ? 'login' : 'signup') }}>
