...
export default function OrganiserManage() {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      const { data, error: invokeError } = await supabase.functions.invoke('manage-event', {
        body: { token, action: 'rsvps' },
      })
      if (invokeError || data?.error) {
        setError(data?.error || 'Impossible d’ouvrir cet espace de gestion.')
      } else {
        setEvent(data.event)
        setRsvps(data.rsvps || [])
      }
  ...
  if (error || !event) {
    return <main className="cl-shell"><div className="cl-container"><section className="cl-panel"><h1>Espace organisateur</h1><p className="cl-form-error">{error || 'Événement introuvable.'}</p><button className="cl-primary-button" type="button" onClick={() => navigate('/organiser')}>Créer un événement</button></section></div></main>
  }
        ...
        <section className="cl-panel">
          ...
          {event.mode === 'invitation' && (
            <div className="cl-rsvp-list">
              <h2>Réponses</h2>
              {rsvps.length === 0 ? <p className="cl-empty">Aucune réponse pour le moment.</p> : rsvps.map((rsvp) => (
                <article className="cl-rsvp-item" key={rsvp.id}>
                  <div><strong>{rsvp.name}</strong>{rsvp.message && <p>{rsvp.message}</p>}</div>
